...
const HUBS = [
  {
    ...
    description: "Hub estratégico para el Cono Sur. Conecta con Bolivia, Paraguay, Argentina, Uruguay y el norte de Chile en las rutas más eficientes desde Asia.",
  ...
  },
  {
    ...
    description: "Plataforma de importación y distribución para Perú y los mercados andinos. Zona franca con procedimientos aduaneros propios para almacenamiento y redistribución.",
...
export default function WingsFreeZone() {
  ...
  return (
    <section id="zonas-francas" className="bg-[#FAFAFA] py-24 md:py-32">
      <div className="max-w-7xl mx-auto px-6">
        ...
        {/* Hub cards */}
        <div ref={hubsRef as React.RefObject<HTMLDivElement>} className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
          {HUBS.map((hub) => (
            <div key={hub.city} className="stagger-item bg-white rounded-2xl shadow-[0_2px_20px_rgba(0,0,0,0.07)] overflow-hidden">
              ...
              <div className="p-8">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {hub.markets.map((m) => (
                      <span key={m} className="text-xs bg-[#C4933F]/10 text-[#C4933F] font-medium px-3 py-1.5 rounded-full" style={{ fontFamily: "var(--font-body)" }}>
                        {m}
                      </span>
                    ))}
                  </div>
        ...
        {/* Benefits grid */}
        <div ref={benefitsRef as React.RefObject<HTMLDivElement>} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {BENEFITS.map((b) => (
            <div key={b.label} className="stagger-item bg-white rounded-xl shadow-[0_1px_8px_rgba(0,0,0,0.05)] p-5 flex gap-4 items-start">
