      ...
      {isOpen && (
        <div className="absolute left-0 top-full z-[100] flex w-[780px] -translate-x-4 overflow-hidden rounded-2xl border border-white/10 bg-[#001240] shadow-[0_24px_64px_rgba(0,0,0,0.4)]">
          ...
          <div className="flex-1 max-h-[480px] overflow-y-auto p-7">
            ...
            {(() => {
              const activeSubtypes = activeCategory?.subtypes.filter((s) => !s.comingSoon) ?? [];
              if (activeSubtypes.length === 0) {
                return (
                  <div className="flex flex-col items-start gap-4 py-4">
                    <span className="rounded-full border border-[#C4933F]/40 px-3 py-1 text-[10px] font-semibold uppercase tracking-widest text-[#C4933F]/60" style={{ fontFamily: "var(--font-body)" }}>
                      Próximamente
                    </span>
                    <p className="text-sm text-white/40 max-w-xs leading-relaxed" style={{ fontFamily: "var(--font-body)" }}>
                      Esta categoría está en preparación. Podemos importar cualquier unidad directamente desde fábrica.
                    </p>
                    <Link
                      href="/importacion"
