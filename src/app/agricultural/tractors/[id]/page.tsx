...
import FloatingModelActions from "@/components/listings/FloatingModelActions";
...
export default async function TractorDetailPage({ params }: Props) {
  ...
  const waLink      = `https://wa.me/${waNumber}?text=${encodeURIComponent(`Hola, quisiera conversar sobre el ${listingTitle}. ¿Pueden ayudarme con información y precio landed total?`)}`;
  const cotizarHref = `/cotizar?modelo=${encodeURIComponent(listing.model)}&marca=${encodeURIComponent(listing.brand)}`;
  ...
  return (
    ...
    <div className="min-h-screen bg-[#FAFAFA] pb-8 md:pb-0">
          ...
          <div className="mt-10 lg:mt-0">
            <div className="sticky top-24 space-y-4">
              ...
              <a
                ...
                className="flex w-full items-center justify-center gap-2.5 rounded-2xl py-4 text-sm font-semibold text-white transition-opacity hover:opacity-90"
      ...
      <FloatingModelActions waLink={waLink} cotizarHref={cotizarHref} />
