...
the `pdoc` grid/layout/logo from the quotation deliverables. Terminology is
aligned to the reference Prado (gasoline) comparison sheet where the same
field exists. Self-contained HTML (logo inlined). Run:
...
# ── Section groups: (título, [(etiqueta, valor), ...]) ─────────────────────
SECTIONS = [
    ("Identificación", [
        ("Modelo", MODEL_NAME),
        ("Versión", MODEL_TRIM),
    ...
    ("Motor de Combustión y Transmisión", [
    ...
    ("Carrocería, Frenos y Neumáticos", [
    ...
    ("Equipamiento Exterior e Iluminación", [
    ...
    ("Tecnología e Inteligencia Vehicular", [
...
def section_html(title: str, rows: list[tuple[str, str]]) -> str:
    grid = "\n".join(
  ...
  <div class="pdoc-spec-section">
    <div class="pdoc-section-bar">{title}</div>
...
SECTIONS_HTML = "\n".join(section_html(title, rows) for title, rows in SECTIONS)
HERO_HTML = "\n      ".join(
...
<style>
  ...
  .pdoc {{
    ...
    padding: 22px 52px 26px; background: #ffffff; color: var(--pd-ink);
  ...
  /* Model identity hero card */
  .pdoc-hero {{ border: 1px solid var(--pd-line); padding: 14px 16px; margin-bottom: 12px; break-inside: avoid; }}
  .pdoc-hero-name {{ font-size: 18px; font-weight: 700; letter-spacing: -0.01em; }}
  .pdoc-hero-trim {{ margin-top: 2px; font-size: 12px; color: var(--pd-muted); }}
  .pdoc-hero-stats {{ display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; margin-top: 12px; }}
  .pdoc-hero-stat {{ border-top: 2px solid var(--pd-ink); padding-top: 6px; }}
  .pd-hero-label {{ display: block; font-size: 9.5px; letter-spacing: 0.06em; text-transform: uppercase; color: var(--pd-muted); }}
  .pd-hero-value {{ display: block; margin-top: 2px; font-family: var(--font-mono, monospace); font-weight: 700; font-size: 14px; font-variant-numeric: tabular-nums; }}
