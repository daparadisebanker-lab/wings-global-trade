...
Willy (Cuzco) listing the TWO Hilux manual-trim models — Full Manual 4x4
and Básica Manual 4x4 — side by side, per-unit pricing only, no summed
grand total (same table format used for the Arompech/Pinares/Rojas
3-model cotizaciones, minus the Overland Plus row since only these two
versions were requested here).
...
(shared cost inputs; both models use the engine's raw 10%-margin output,
no rounding adjustment — that adjustment is specific to the Overland Plus
model only).

Client provided: Willy (first name only) · Cuzco. No apellido, no DNI, no
empresa/RUC were given, so none are shown (house rule: never fabricate
client data).
...
DOC_NUMBER = "COT-WGT-2026-0914-F"
FECHA = "Lima, 14-09-2026"

CLIENTE_NOMBRE = "Willy"
CLIENTE_CIUDAD = "Cuzco"
