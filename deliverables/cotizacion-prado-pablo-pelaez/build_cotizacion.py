...
Toyota Land Cruiser Prado 2026 2.4T Híbrido (FULL), 1 unidad, issued to
Pablo Peláez Guerrero (DNI 33660420, Chiclayo, Perú), port of arrival
Callao, Perú.

Pricing: same per-unit landed-cost basis already established for this
exact vehicle on the Atilio Gargate quotation (deliverables/
cotizacion-prado-atilio-gargate/build_cotizacion.py) — sourced from
FORMULA_DE_COSTOS_1.xlsx, sheet "PRADO", column D — at the same 8%
margin. No new cost/freight data was supplied for a single-unit order,
so the standing per-unit figure is reused as-is; UNITS is simply 1 here
instead of 2 (this is a referencial price, same house-rule disclosure
in Observaciones as the other quote).
  Costo total de importación (landed, pre-IGV) = USD 83,569.50/unidad
  Margen 8% = USD 6,685.56/unidad -> Valor de venta = USD 90,255.06/unidad

Vendedor: same Peru issuing entity as all Peru cotizaciones — IMP. Y EXP.
WINGS AUTOMÓVILES S.R.L., RUC 20532415650. Contact updated with the
Peruvian WhatsApp Business number from the client-shared screenshot
("Wings Global Trade (You)" / "~Wings Global Trade Perú"): +51 958 381
473 — added to the Vendedor block and footer (the Atilio Gargate quote
didn't carry a phone number).

Reuses the `pdoc` grid/layout from the Wings quotation family (icon-only
logo, "WINGS GLOBAL TRADE" letterhead, centered section bars) — same
element set as the Atilio Gargate Prado quote, plus DNI/Ciudad added to
the client party box. Run:
...
DOC_NUMBER = "COT-WA-2026-0909"
DOC_DATE = "09-09-2026"

CLIENTE = "Pablo Peláez Guerrero"
DNI_CLIENTE = "33660420"
CIUDAD_CLIENTE = "Chiclayo"

VENDEDOR_WHATSAPP = "+51 958 381 473"

# ── Source: FORMULA_DE_COSTOS_1.xlsx, sheet "PRADO", column D ─────────────
# Costo total de importación (landed cost, pre-nationalization IGV) = D60,
# same per-unit figure already established on the Atilio Gargate quote.
...
<title>Cotización · Wings Automóviles · Toyota Land Cruiser Prado 2026 · Pablo Peláez</title>
<style>
  :root {{
  ...
  .pdoc-rule {{ position: relative; height: 3px; margin: 14px 0 14px; background: var(--pd-line); }}
  ...
  .pdoc-parties {{ display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 14px; }}
  .pdoc-party {{ border: 1px solid var(--pd-line); padding: 12px 15px; }}
  ...
  .pdoc-totals {{ margin: 16px 0 6px auto; width: 360px; }}
...
<article class="pdoc">
  ...
  <div class="pdoc-parties">
    <div class="pdoc-party">
      ...
      <dl class="pdoc-party-meta">
        ...
        <dt>WhatsApp</dt><dd>{VENDEDOR_WHATSAPP}</dd>
      </dl>
    </div>
    <div class="pdoc-party">
      ...
      <dl class="pdoc-party-meta">
        <dt>DNI</dt><dd>{DNI_CLIENTE}</dd>
        <dt>Ciudad</dt><dd>{CIUDAD_CLIENTE}</dd>
        <dt>País</dt><dd>Perú</dd>
      </dl>
    </div>
  </div>
  ...
  <footer class="pdoc-footer">
    <div>
      ...
      <div>WhatsApp {VENDEDOR_WHATSAPP}</div>
    </div>
