...
Inversiones Arompech S.R.L. (rep. Arnulfo Aroni Miranda) listing THREE
Hilux models side by side, per-unit pricing only — no summed grand total,
per explicit client request ("Don't do the sum, it's simply a table with
the price of each of the three models").

Cost basis — same SUNAT import-cost engine and identical shared cost
inputs (flete internacional, Zofratacna, portuarios, agencia, seguro, IGV,
percepción, T.C., margen 10%) as the standing Hilux Travo Overland Plus
cotización; only FOB differs per model. Recorded in
`deliverables/PRICING_NOTES.md` as the standing knowledge base for these
three FOB values:
  · Toyota Hilux Travo Overland Plus  — FOB 43,000.00 (existing)
  · Toyota Hilux Full Manual 4x4 (trim 2.8 4x4 V M/T) — FOB 44,205.00 (user-specified 2026-09-14)
  · Toyota Hilux Básica Manual 4x4 (trim 2.8 4x4 E M/T) — FOB 36,000.00 (user-specified 2026-09-14)

The Overland Plus row reuses its standing, already-delivered per-unit sale
price (55,211.87 / 65,150.01) — which carries a one-off +48.24/unit
commercial rounding adjustment from a prior deal — rather than the engine's
raw output, for consistency with every other Overland Plus cotización
issued. Full Manual 4x4 and Básica Manual 4x4 are fresh, first-time
quotes, so they use the engine's raw 10%-margin output with no rounding
adjustment (see PRICING_NOTES.md for the full rationale).

Model names match the existing fichas técnicas exactly:
  · ficha-tecnica-hilux-2.8-4x4-v-mt/ → "Toyota Hilux Full Manual 4x4" (FT-WGT-2026-0907)
  · ficha-tecnica-hilux-basica-manual-4x4/ → "Toyota Hilux Básica Manual 4x4" (FT-WGT-2026-0908)
...
DOC_NUMBER = "COT-WGT-2026-0914"
FECHA = "Lima, 14-09-2026"

CLIENTE_EMPRESA = "INVERSIONES AROMPECH S.R.L."
CLIENTE_RUC = "20535835986"
CLIENTE_REPRESENTANTE = "Arnulfo Aroni Miranda"
CLIENTE_DNI = "08294636"
CLIENTE_DIRECCION = "Av. La Cultura 808, Interior pabellón C - puesto 017"
CLIENTE_CIUDAD = "Lima, Santa Anita"
...
<article class="pdoc">
  ...
  <div class="pdoc-parties">
    ...
    <div class="pdoc-party">
      ...
      <div class="pdoc-party-name">{CLIENTE_EMPRESA}</div>
      <dl class="pdoc-party-meta">
        <dt>RUC</dt><dd>{CLIENTE_RUC}</dd>
        <dt>Representante</dt><dd>{CLIENTE_REPRESENTANTE} (DNI {CLIENTE_DNI})</dd>
