...
1x Toyota Land Cruiser Prado Híbrida, client Roy Torres (DNI 73471021,
Cerro de Pasco).

Same pricing basis as cotizacion-land-cruiser-prado-hibrida/build_cotizacion.py
(ex-Miami dealer-invoice landed-cost buildup, verified against the
source costing sheet):

  Goods Value           73,523.20
  Ocean Freight          3,250.00
  Cargo Insurance (1% of Goods Value)  735.23
  CIF                   77,508.43
  IGV importación (18%) 13,951.52
  Gastos Portuarios        375.00
  Agencia de Aduana        300.00
  Subtotal              92,134.95
  Octavio fee            2,000.00
  TOTAL LANDED COST     94,134.95
  Markup 6.3%             5,930.50
  SELLING PRICE         100,065.45  ← IGV-inclusive (per client correction)

Client-facing: Valor de Venta and IGV are back-calculated from the
IGV-inclusive Selling Price (÷1.18) rather than adding a fresh 18% on
top — Precio Total matches the source sheet's Selling Price exactly.
...
DOC_NUMBER = "COT-WGT-2026-0923-B"
...
selling_price = r2(landed_cost + margin_usd)  # Precio total, YA incluye IGV

# ── Client-facing: back out Valor de Venta / IGV from the IGV-inclusive
# Selling Price ───────────────────────────────────────────────────────
