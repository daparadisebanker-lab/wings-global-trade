...
3x Toyota Hilux Básica Manual 4x4, client Epifanio Coila Paricahua
(DNI 06253969).

Standing per-unit price from PRICING_NOTES.md (engine's raw 10%-margin
output, FOB 36,000.00, no rounding adjustment): 47,348.13/unit. Order-style
quotation with quantity and summed grand total (3 units), same format as
the Lucio Reyes 4-unit cotización. No city/empresa data was provided, so
only name and DNI are shown (house rule: never fabricate client data).
...
DOC_NUMBER = "COT-WGT-2026-0921-B"
FECHA = "Lima, 21-09-2026"

CLIENTE = "Epifanio Coila Paricahua"
CLIENTE_DNI = "06253969"
...
VALOR_UNIT = compute_sale_price(FOB_BASICA_MANUAL)
UNITS = 3

IMPORTE = r2(VALOR_UNIT * UNITS)
IGV = r2(IMPORTE * IGV_RATE)
PRECIO_TOTAL = r2(IMPORTE + IGV)
...
<title>Cotización · Wings Global Trade · Toyota Hilux Básica Manual 4x4</title>
<style>
  :root {{
...
<article class="pdoc">
  ...
  <table class="pdoc-table">
    ...
    <tbody>
      <tr>
        ...
        <td class="pd-cell-num">{fmt(VALOR_UNIT)}</td>
  ...
  <div class="pdoc-totals">
    <div class="pdoc-total-row">
      ...
      <span class="pd-total-value">{fmt(IMPORTE)}</span>
    </div>
    <div class="pdoc-total-row">
...
print(f"units={UNITS} valor_unit={fmt(VALOR_UNIT)} importe={fmt(IMPORTE)} igv={fmt(IGV)} precio_total={fmt(PRECIO_TOTAL)} (S/ {fmt(PRECIO_TOTAL_SOLES)})")
