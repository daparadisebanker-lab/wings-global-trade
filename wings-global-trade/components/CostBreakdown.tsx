        ...
        <div className="grid grid-cols-1 lg:grid-cols-[3fr_2fr] gap-8 items-start">
          ...
          <div className="bg-white rounded-2xl shadow-[0_2px_20px_rgba(0,0,0,0.07)] overflow-hidden">
            {COST_ITEMS.map((item, i) => (
              <div
                key={item.n}
                className={`flex gap-5 px-7 py-5 ${i < COST_ITEMS.length - 1 ? "border-b border-[#F8F6F0]" : ""} ${i % 2 === 1 ? "bg-[#FDFCFB]" : ""}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-4 mb-1">
                    <p className="text-[#1C1A16] text-sm font-semibold">{item.label}</p>
          ...
          {/* Right: sticky callout stack */}
          <div className="lg:sticky lg:top-24 space-y-4">
            {/* Main callout */}
            <div className="bg-[#0D1B2A] rounded-2xl p-8">
              <p className="text-[#C4933F] text-xs font-medium tracking-[0.12em] uppercase mb-5">
                La diferencia Wings
              </p>
              <p
            ...
            <div className="bg-white rounded-2xl shadow-[0_2px_20px_rgba(0,0,0,0.07)] p-7">
              <p className="text-[#1C1A16] text-xs font-semibold tracking-[0.12em] uppercase mb-5">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={item} className="flex items-center gap-3">
                    ...
                    <p className="text-[#6B6560] text-sm">{item}</p>
                  </div>
                ))}
              </div>
            </div>

            <p className="text-[#6B6560] text-xs leading-relaxed px-1">
