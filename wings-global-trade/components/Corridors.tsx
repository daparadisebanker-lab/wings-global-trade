const CORRIDORS = [
  ...
  {
    hub: "Directo",
    location: "Buenaventura / Cartagena",
    badge: "Colombia",
    description: "Importación directa sin tránsito por zona franca. Ruta más eficiente para el mercado colombiano.",
    routes: [
      { dest: "Bogotá", time: "28–35 días", via: "Buenaventura → troncal andina" },
      { dest: "Medellín", time: "28–35 días", via: "Buenaventura / Cartagena" },
      { dest: "Cali", time: "26–32 días", via: "Buenaventura (puerto directo)" },
    ],
  },
];
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-white/50 text-lg mt-5 max-w-xl mx-auto">
            Tránsito marítimo Asia → Hub: <span className="text-[#C4933F] font-medium">25–35 días</span>. Los tiempos abajo incluyen distribución al destino final.
          </p>
        </div>
            ...
            >
              ...
              <div className="p-7 border-b border-white/8">
                <div className="flex items-start justify-between mb-4">
                  <span className="inline-block bg-[#C4933F]/15 text-[#C4933F] text-xs font-semibold tracking-[0.12em] uppercase rounded-full px-3 py-1">
                    {c.badge}
                  </span>
                </div>
                <h3
              ...
              <div className="flex-1 p-7">
                <p className="text-white/30 text-xs font-medium tracking-[0.15em] uppercase mb-5">
                  Destinos y tránsito
                </p>
                <div className="space-y-0">
                  {c.routes.map((r, i) => (
                    <div
                      key={r.dest}
                      className={`py-3.5 ${i < c.routes.length - 1 ? "border-b border-white/[0.06]" : ""}`}
                    >
                      <div className="flex items-center justify-between mb-1">
                        <p className="text-white/85 text-sm font-medium">{r.dest}</p>
                        <p className="text-[#C4933F] text-xs font-mono">{r.time}</p>
                      </div>
                      <p className="text-white/30 text-xs">{r.via}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <p className="text-center text-white/25 text-xs mt-10 max-w-xl mx-auto leading-relaxed">
          Tiempos no incluyen despacho aduanero en destino (3–10 días adicionales). Productos con restricciones especiales pueden requerir tiempo adicional de despacho.
        </p>
      </div>
    </section>
  );
