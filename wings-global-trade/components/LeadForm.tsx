...
export default function LeadForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) { const data = await res.json(); setError(data.error || "Error al enviar. Intenta de nuevo."); return; }
      setSubmitted(true);
    } catch { setError("Error de red. Por favor intenta de nuevo."); } finally { setLoading(false); }
  }
    ...
    <section id="contacto" className="bg-[#F8F6F0] py-24 md:py-32">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[2fr_3fr] gap-8 max-w-6xl mx-auto">
          {/* Left: logo + trust points */}
          <div ref={trustRef as any} className="flex flex-col gap-5">
            {/* logo2-v2 — only shown on desktop where the two-column layout gives it context */}
            <div className="hidden lg:block mb-2">
              <img
                src="/wings-logo2-v2.svg"
            ...
            {[
              ...
              <div
                key={item.label}
                className="stagger-item bg-white rounded-2xl shadow-[0_2px_16px_rgba(0,0,0,0.06)] p-6 flex gap-4 items-start"
              >
    ...
    <div className="text-center py-8">
      ...
      <div className="space-y-3 max-w-sm mx-auto">
        <a href={CALENDLY_URL} target="_blank" rel="noopener noreferrer"
          className="flex items-center justify-center gap-2 w-full bg-[#001E50] hover:bg-[#0A2D6E] text-white font-semibold py-3.5 rounded-full text-sm transition-colors"
          style={{ fontFamily: "var(--font-body)" }}>
          <CalendarIcon /> Agendar llamada de 30 minutos
        </a>
        <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer"
          className="flex items-center justify-center gap-2 w-full border-2 border-[#C4933F] hover:bg-[#C4933F]/5 text-[#C4933F] font-semibold py-3.5 rounded-full text-sm transition-colors"
          style={{ fontFamily: "var(--font-body)" }}>
...
function Field({ label, name, type, placeholder, value, onChange, required }: { label: string; name: string; type: string; placeholder: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; required: boolean }) {
  return (
    <div>
      <label className="block text-[#1C1A16] text-xs font-semibold tracking-[0.08em] uppercase mb-2" style={{ fontFamily: "var(--font-body)" }}>{label}</label>
