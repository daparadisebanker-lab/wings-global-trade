...
export default function WhyWings() {
  return (
    <section className="bg-[#F8F6F0] py-28 md:py-36">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-[#6B6560] text-lg mt-5 max-w-xl mx-auto">
            Un agente de carga mueve tu carga. Wings gestiona todo el proceso.
          </p>
        </div>

        {/* Mobile: Wings-only feature list */}
        <div className="md:hidden bg-white rounded-2xl shadow-[0_2px_24px_rgba(0,0,0,0.08)] overflow-hidden">
          <div className="px-6 py-4 border-b border-[#F0EDE6] bg-[#C4933F]/5">
            <p className="text-[#C4933F] text-xs font-semibold tracking-[0.12em] uppercase">Wings Global Trade — Todo incluido</p>
          </div>
          {FEATURES.map((feat, i) => (
            <div key={feat.label} className={`flex items-start gap-4 px-6 py-4 ${i < FEATURES.length - 1 ? "border-b border-[#F8F6F0]" : ""}`}>
              ...
              <p className="text-[#1C1A16] text-sm leading-snug">{feat.label}</p>
            </div>
          ))}
        </div>
        ...
        <div className="hidden md:block bg-white rounded-2xl shadow-[0_2px_24px_rgba(0,0,0,0.08)] overflow-hidden">
          {/* Column headers */}
          <div className="grid grid-cols-[2fr_1fr_1fr_1fr] border-b border-[#F0EDE6]">
            <div className="px-8 py-5" />
            <div className="px-4 py-5 text-center border-l border-[#F0EDE6] bg-[#C4933F]/5">
              <p className="text-[#C4933F] text-xs font-semibold tracking-[0.12em] uppercase">Wings</p>
            </div>
            <div className="px-4 py-5 text-center border-l border-[#F0EDE6]">
              <p className="text-[#6B6560] text-xs font-medium tracking-[0.1em] uppercase">Agente de carga</p>
            </div>
            <div className="px-4 py-5 text-center border-l border-[#F0EDE6]">
              <p className="text-[#6B6560] text-xs font-medium tracking-[0.1em] uppercase">DIY</p>
            </div>
          </div>
          ...
          {FEATURES.map((feat, i) => (
            <div
              key={feat.label}
              className={`grid grid-cols-[2fr_1fr_1fr_1fr] ${i < FEATURES.length - 1 ? "border-b border-[#F8F6F0]" : ""} ${i % 2 === 1 ? "bg-[#FDFCFB]" : ""}`}
            >
              <div className="px-8 py-4 flex items-center">
                <p className="text-[#1C1A16] text-sm leading-snug">{feat.label}</p>
              </div>
              <div className="border-l border-[#F0EDE6] bg-[#C4933F]/[0.03] flex items-center justify-center py-4">
                <Check value={feat.wings as Indicator} highlight />
              </div>
              <div className="border-l border-[#F0EDE6] flex items-center justify-center py-4">
          ...
          <div className="flex items-center gap-6">
            {[
              ...
              <div key={label} className="flex items-center gap-2">
                {icon}
                <span className="text-[#6B6560] text-xs">{label}</span>
              </div>
            ))}
          </div>
          <a
