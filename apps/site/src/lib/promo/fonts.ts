...
// by the fonts' internal family names, so we hand it the actual files. Mirror of
// apps/tower/src/lib/promo/fonts.ts — same cards, same fonts. Kept in
// apps/site/public/fonts and pinned into the function bundle via next.config
// outputFileTracingIncludes. Missing files are filtered out at call time so the
// OG route always renders (falling back to system fonts).
