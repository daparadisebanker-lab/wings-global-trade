...
export function BrandMarquee() {
  ...
  return (
    ...
    >
      ...
      {/* Track */}
      <div className="relative overflow-hidden border-t border-b border-warm-white/[0.06] py-8 md:py-8 brand-marquee-wrap">
        {/* Left fade */}
        <div
          ...
          {items.map((brand, i) => (
            <span key={`${brand.name}-${i}`} className="flex items-center shrink-0">
              <Image
                ...
                height={40}
                draggable={false}
                className="h-[64px] w-auto max-w-[200px] md:h-[52px] md:max-w-[180px] object-contain select-none"
                style={{ filter: 'brightness(0) invert(1)', opacity: 0.65 }}
                unoptimized
              />
              <span
                className="mx-12 h-1 w-1 rounded-full shrink-0"
