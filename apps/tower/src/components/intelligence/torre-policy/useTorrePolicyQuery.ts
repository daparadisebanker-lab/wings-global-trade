...
import { listCostingLanes, type CostingLane } from '@/lib/actions/costing'
import { getTorrePolicy, type TorrePolicy } from '@/lib/actions/torre-policy'

export function useTorrePolicyLanesQuery() {
  return useQuery<CostingLane[], Error>({
    queryKey: ['tower', 'intelligence', 'torre-policy', 'lanes'],
    queryFn: async () => {
      const r = await listCostingLanes()
