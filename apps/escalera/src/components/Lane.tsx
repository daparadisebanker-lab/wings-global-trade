...
//   left   PASS — the booth dims to --dimmed and the lane advances
...
import { LANE, skusOf } from '@/lib/catalogue'
import {
...
export function Lane({ onOpenSku }: { onOpenSku: (sku: Sku) => void }) {
  ...
  return (
    <div className="relative flex h-dvh flex-col bg-lane-ground text-surface">
      <div {...bind()} className="relative flex-1 touch-pan-y overflow-hidden">
        <div ref={areaRef} className="h-full">
          <div
            ref={cardRef}
            className={`flex h-full flex-col will-change-transform transition-opacity duration-light ${
              passed ? 'opacity-dimmed' : 'opacity-lit'
            }`}
          >
            <div
              className={`flex h-full flex-col px-5 pt-6 transition-opacity duration-cross ${
                swapping ? 'opacity-0' : 'opacity-100'
              }`}
            >
              <div className="flex items-baseline justify-between">
                <p className="stamp opacity-resting">
                  Pasillo {String(index + 1).padStart(2, '0')} / {LANE.length}
                </p>
                {passed && <p className="stamp opacity-resting">Pasada</p>}
              </div>
              ...
              <div className="mt-5 min-h-0 flex-1 overflow-y-auto pb-4">
                <div className="grid grid-cols-3 gap-2">
                  {faces.slice(0, BOOTH_FACES).map((sku) => (
                    <button
                      ...
                      className="relative aspect-square overflow-hidden rounded-record bg-surface-2"
                    >
                      ...
                      {rec.isSkuSelected(sku.series_uid, sku.sku_uid) && (
                        <>
                          ...
                          <span aria-hidden className="collected-corner" />
                        </>
                      )}
                    </button>
                  ))}
                ...
                {faces.length > BOOTH_FACES && (
                  <p className="stamp mt-3 opacity-resting">
            ...
            <div
              ...
              aria-hidden
              style={{ opacity: 0 }}
              className="pointer-events-none absolute inset-0 ring-4 ring-inset ring-surface"
            >
              <span className="stamp absolute right-5 top-5 bg-surface px-3 py-1.5 text-ink">
                Guardar serie
              </span>
            </div>
            <div
            ...
            >
              <span className="stamp absolute left-5 top-5 border border-surface/40 px-3 py-1.5">
                Pasar
              </span>
            </div>
          </div>
...
function LaneActions({
  ...
  return (
    <div className="flex shrink-0 items-center gap-3 px-5 pb-24 pt-3 pr-12">
      <button
        type="button"
        onClick={onPass}
        className="flex-1 rounded-chrome border border-surface/30 py-3.5 text-t0"
      >
        Pasar
      </button>
      <button
        type="button"
        onClick={onCollect}
        className={`flex-1 rounded-chrome py-3.5 text-t0 ${
          collected ? 'border border-surface/30 opacity-resting' : 'bg-surface text-ink'
        }`}
      >
...
function EdgeScrubber({ index, onGo }: { index: number; onGo: (i: number) => void }) {
  ...
  return (
    <div
      {...bind()}
      role="slider"
      aria-label="Recorrer el pasillo"
      aria-valuemin={1}
    ...
    >
      <div ref={railRef} className="my-6 mr-2 flex w-[3px] flex-col justify-between">
        {LANE.map((s: Series, i) => {
          ...
          return (
            <span
              key={s.series_uid}
              aria-hidden
              className={`block w-full transition-all duration-light ${
                i === index ? 'h-4 bg-surface' : 'h-2'
              } ${
                isCollected
                  ? 'bg-surface'
                  : isPassed
                    ? 'border border-surface/40 bg-transparent'
                    : 'bg-surface/25'
              }`}
            />
          )
