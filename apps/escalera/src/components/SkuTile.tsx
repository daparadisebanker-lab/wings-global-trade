...
import { finishLabel, patternLabel } from '@/lib/catalogue'
import { haptic } from '@/lib/record'
...
export function SkuTile({
  ...
  return (
    <figure
      className={`m-0 transition-opacity duration-light ${dimmed ? 'opacity-dimmed' : 'opacity-lit'}`}
    >
      <button
        type="button"
        onPointerDown={start}
        ...
        className="relative block aspect-square w-full select-none overflow-hidden rounded-record bg-surface-2"
      >
        ...
        {collected && (
          <>
            <span aria-hidden className="collected-mark pointer-events-none absolute inset-0" />
      ...
      {showCaption && (
        <figcaption className="pt-1.5">
          {/* the code is what a buyer pastes into WhatsApp at 11pm — mono, always */}
          <p className="mono text-[11px] leading-tight opacity-lit">{sku.code}</p>
          <p className="text-[10px] leading-tight opacity-resting">{finishLabel(sku)}</p>
          <p className="text-[10px] leading-tight opacity-dimmed">{patternLabel(sku)}</p>
        </figcaption>
      )}
    </figure>
  )
