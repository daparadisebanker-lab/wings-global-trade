...
import { formatLabel } from '@/lib/catalogue'
...
export function SpecBar({
  ...
  return (
    <header className={invertedOnDark ? 'text-surface' : 'text-ink'}>
      <h2 className="font-display text-t2 font-semibold leading-tight tracking-[-0.02em]">
        {series.name_raw}
      </h2>
      <p className="stamp mt-1 opacity-resting">
        {formatLabel(series)}×{series.thickness_mm}MM
      </p>

      {/* the inverted bar itself */}
      <div
        className={`mt-3 flex h-8 items-center px-3 ${
          invertedOnDark ? 'bg-surface text-ink' : 'spec-bar'
        }`}
      >
