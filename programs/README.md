...
| `automobiles/` | Car-first pivot — the `automoviles` catalog category (31 model lines / 76 trims / 11 brands, sourced from the fleet costing sheet, no pricing — that stays in Rowe) promoted to lead the nav; open question is whether it stays a pre-lane category or becomes a coded EQUIPMENT-archetype lane | **Phase-0 tactical build SHIPPED** (catalog data + nav promotion, 2026-08-23); **strategic pivot (lane status, photography, homepage) is a PROPOSAL** pending Muaaz | `SCOPE.md` |
