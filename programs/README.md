...
| `automobiles/` | **WGT/07 Automóviles** — car-first pivot, now a coded EQUIPMENT-archetype lane (dual unit math: per unit + per container; dual taxonomy: segment-canonical, brand-overlay; ion-blue/asphalt livery); 31 model lines / 76 trims / 11 brands sourced from the fleet costing sheet, no pricing — that stays in Rowe | **Phase 0 (qualification) + Phase 1 (registration) + Phase 2 (livery) COMPLETE** 2026-08-23 — `lane.config.ts` + `livery.css` + registry ledger written; **Phase 3 (route migration off /catalogo/automoviles) still pending, nothing renders the new livery yet** | `SCOPE.md` → `../../packages/liveries/automoviles/lane.config.ts` |
