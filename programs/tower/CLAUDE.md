...
**High-water mark (update when you add migrations):** max claimed = **`tower_55`** (`issuing_entities` — file `20260724160000`, committed on branch `claude/branded-proforma-pricing-0jyzbx`; apply on deploy) · next free = **`tower_56`**. The Mister Torre workstream claimed **`tower_48–54`** (`torre_artifacts` · `rate_tables` · `tariff_positions` · `org_rules` · `watch_signals` · `knowledge_corpus` · `torre_sends`). Reserved-but-unbuilt: `tower_27–29` (RB console), `tower_37` (journey-automation slot). `tower_39` is deliberately double-used (`rep_profiles` + PR #4's `status_transition_guards`); the timestamp collision was reconciled in **PR #6** (PR #4's four migrations re-timestamped to `20260722…`).
