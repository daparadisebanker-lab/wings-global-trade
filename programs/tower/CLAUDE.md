...
**High-water mark (update when you add migrations):** max claimed = **`tower_56`** (`issuing_entities` — file `20260724235523`, applied to prod; on branch `claude/branded-proforma-pricing-0jyzbx`) · next free = **`tower_57`**. The Mister Torre workstream claimed **`tower_48–54`** (`torre_artifacts` · `rate_tables` · `tariff_positions` · `org_rules` · `watch_signals` · `knowledge_corpus` · `torre_sends`). **`tower_55` (`team_space`, prod version `20260724160000`) is applied to prod but its migration file is NOT yet in this branch — a drift; recover it before the next `db push`** (this is why `issuing_entities` renumbered 55→56 after a ledger cross-check). Reserved-but-unbuilt: `tower_27–29` (RB console), `tower_37` (journey-automation slot). `tower_39` is deliberately double-used (`rep_profiles` + PR #4's `status_transition_guards`); the timestamp collision was reconciled in **PR #6** (PR #4's four migrations re-timestamped to `20260722…`).
